// mis é jour le 2014-09-04 par MTE : ajout de tous les $b du 200 maintenant qu'ils sont contrélés
// mis é jour le 2014-10-07 par MTE : remplacement des 200$b par 181 et si possible 182
// mis é jour le 2015-03-19 par IAN : mise é jour de la date dans le script CAT_creerExemplaireRetro, passée de 2014 é 2015
// mis é jour le 2015-04-24 par IAN : mise é jour de la date dans le script CAT_creer-ThesesImpEditionCommerciale, mentions subdivision de forme Théses et écrits académiques ($3 ppn) enlevées des zones 606
// mis é jour le 2015-12-07 par SRY : CAT_creerTheseImprimeReproduction : supprimer 451 et 452
...
// mis é jour le 2016-02-08 par SRY : CAT_creerElectronique, CAT_creerPeriodElectr : 230,337,339
// mis é jour le 2016-02-22 par SRY : CAT_creerCollection : ajout zone 301
// mis é jour le 2016-11-14 par SRY :
...
// mis é jour le 2017-03-16 par SRY :
// 				Nouvelles consignes RDA FR 2017 (zones 183, 210, 219, 215
// mis é jour le 2018-01-03 par SRY : création de 2 scripts :
// 				CAT_creerEchantillonAccompagne et CAT_creerObjet
// mis é jour le 2018-05-31 par SRY : création de script :
// 				CAT_creerArticleImp
//				modification du vocabulaire (vedette matiére transformé en point d'accés)
// mis é jour le 2020-01-01 par SRY : modification pour évolutions Unimarc
...
// mis é jour le 2022-04-13 par LJ : création de deux scripts de création de mémoire nativement numérique
//				CAT_creerMemoireElec et CAT_creerMemoireImpr
// mis é jour le 2022-05-25 par MRX :
...
// mis é jour le 2022-06-03 par MRX :
//		création de 3 scripts
//				CAT_creerTheseElecAutreVersionAuteur : création version remaniée d'une thése
//				CAT_creerTheseImprNonDeposee : création thése non déposée
//				CAT_creerTheseImprPerdue : création thése perdue
// mise é jour le 2022-06-22 par MRX :
//				CAT_creerTheseEclectroniqueReproduction
//				CAT_creerHDRElecVO : création HDR nativement électronique
//				CAT_creerHDRElecReproduction : création reproduction électronique de HDR
//				CAT_creerMemoireElec
//				CAT_creerMemoireElecReproduction : création reproduction électronique de mémoire
//				CAT_creerTheseRemanieeAuteur : création version remaniée d'une thése
...
{
	// permet d'ajouter 181Images animéees et 182 é
...
function CAT_creerAtlas()
{
	...
	application.activeWindow.title.insertText(
		...
		"700 #1$aNom Auteur relatif é l'oeuvre ou é l'expression$bPrénom$4180"
	);
...
function CAT_creerCollection()
{
	...
	application.activeWindow.title.insertText(
		...
		"701 #1$aNom co-Auteur relatif é l'oeuvre ou é l'expression$4651"+ "\n" +
		"710 02$a@Nom Collectivité Auteur relatif é l'oeuvre ou é l'expression$4651" + "\n" +
		"711 02$a@Nom Collectivité co-Auteur relatif é l'oeuvre ou é l'expression$4651" + "\n" +
		"712 02$a@Nom Collecteur Auteur relatif é la manifestation ou é l'item$4Code de Fonction"
	);
	application.activeWindow.codedData = true;
}
function CAT_creerCollectivite()
{
	...
	application.activeWindow.title.insertText(
		"008 $aTb5" + "\n" +
		"106 ##$a0$b1$c0" + "\n" +
		"150 ##$aType de collectivité officielle (11 codes possibles, voir le GM)$bCode de congrés ou de conférence (valeur 0 ou 1)" + "\n" +
		"210   $90y$a@Nom de la collectivité$bnom de la collectivité subordonnée facultative$clocalisation facultative" + "\n" +
		"340 ##$aNote biographique (informations é justifier par des sources mentionnées en zone 810)" + "\n" +
...
function CAT_creerElectronique()
{
	...
	application.activeWindow.title.insertText(
		...
		"608 ##$aIndexation Forme ou Genre Rameau$2rameau" + "\n" +
		"700 #1$aNom Auteur relatif é l'oeuvre ou é l'expression$bPrénom$4070"  + "\n" +
		"712 02$a@Nom Collectivité auteur relatif é la manifestation ou é l'item$4Code de Fonction"  + "\n" +
...
{ // Ce script permet de créer un exemplaire avec la mention retro en 991, maj par MTE le 2013-08-29
	...
	application.activeWindow.title.insertText(
		...
		"930 ##$aCote$jCode PEB" + "\n" +
		"991 ##$aRETRO-2016-FINANCEMENT ABES-[Nom du prestataire]-[Code du catalogueur]-CRE SAI$b[Informations nécessaires é la bibliothéque ou au prestataire]" + "\n" +
		"991 ##$aRETRO-2016-FINANCEMENT ABES-[Nom du prestataire]-[Code du catalogueur]-CRE DER$b[Informations nécessaires é la bibliothéque ou au prestataire]" + "\n" +
		"991 ##$aRETRO-2016-FINANCEMENT ABES-[Nom du prestataire]-[Code du catalogueur]-LOC$b[Informations nécessaires é la bibliothéque ou au prestataire]" + "\n" +"\n" +
		"991 ##$aCatalogage rétrospectif-2016-FINANCEMENT ABES-[Nom du prestataire]-[Code du catalogueur]-CRE SAI$b[Informations nécessaires é la bibliothéque ou au prestataire]" + "\n" +
		"991 ##$aCatalogage rétrospectif-2016-FINANCEMENT ABES-[Nom du prestataire]-[Code du catalogueur]-CRE DER$b[Informations nécessaires é la bibliothéque ou au prestataire]" + "\n" +
		"991 ##$aCatalogage rétrospectif-2016-FINANCEMENT ABES-[Nom du prestataire]-[Code du catalogueur]-LOC$b[Informations nécessaires é la bibliothéque ou au prestataire]"
	);
}
...
{ // Ce script permet de créer une notice de document multimedia Za
	...
	application.activeWindow.title.insertText(
		...
		"214 #0$aLieu de publication$bAdresse de l'éditeur$cNom de l'éditeur$dDate de publication [CONSULTER LE GUIDE METHODOLOGIQUE POUR LE BON USAGE DES INDICATEURS ET SOUS-ZONES NECESSAIRES SELON LE TYPE DE MENTION]" + "\n" +
		"215 ##$aImportance matérielle$cMention d'ill.$dDimensions$eMatériel d'accompagnement" + "\n" +
		"215 ##$aImportance matérielle$cMention d'ill.$dFormat$eMatériel d'accompagnement(2éme support).." + "\n" +
		"225  #$a@Titre de la Collection$xISSN$vNuméro" + "\n" +
		"300 ##$aNote générale" + "\n" +
		"305 ##$aNote sur l'édition" + "\n" +
...
{ // Ce script permet de créer une notice de RCR Tw
	...
	application.activeWindow.title.insertText(
		...
		"R140 $aNuméro RCR" + "\n" +
		"R150 $aParticipation é un réseau documentaire autre que le Sudoc" + "\n" +
		...
		"R211 $aFax" + "\n" +
		"R220 $aAdresse Web de la bibliothéque$bAdresse Web du Catalogue en Ligne$dAdresse électronique du service de renseignements" + "\n" +
		...
		"R310 $aHistorique" + "\n" +
		"R410 $aNom des Organismes associés$bNature des liens$cNéRCR$3NéPPN de la notice liée$eCode CR Sudoc-PS$yNéILN_XXX " + "\n" +
		"R440 $aNote sur les organismes associés" + "\n" +
		"R500 $aOuvertures" + "\n" +
		"R510 $aFermetures" + "\n" +
		"R520 $aConditions d'accés" + "\n" +
		"R530 $aNotes particuliéres sur l'accés" + "\n" +
		"R610 $aCode Dewey$bPoint d'accés autorisé - nom commun Rameau" + "\n" +
		"R620 $aDescription de la collection" + "\n" +
		"R800 $aAccés é des Bases de données [Oui/Non]$bNom des BDD$cConditions d'utilisation" + "\n" +
		...
		"R860 $aConsultation sur Place [Oui/Non]$bPrét é Domicile [Oui/Non]$cConditions" + "\n" +
		"R870 $aPortage é domicile [Oui/Non]$bConditions" + "\n" +
		"R880 $aAutres services$bConditions" + "\n" +
		"R890 $aNotes sur les services proposés" + "\n" +
		"R901 $aParticipation é un réseau documentaire$bAutre réseau" + "\n" +
...
{ // Ce script permet de créer une notice de partition Ma
	...
	application.activeWindow.title.insertText(
		...
		"200 1#$a@Titre$eComplément du Titre$f1re mention de responsabilité$gMention de responsabilité de fonction différente" + "\n" +
		"205 ##$aEdition" + "\n" +
		"208 ##$aMention spécifique de musique imprimée (Format)$dMention spécifique paralléle de musique imprimée" + "\n" +
		"214 #0$aLieu de publication$bAdresse de l'éditeur$cNom de l'éditeur$dDate de publication [CONSULTER LE GUIDE METHODOLOGIQUE POUR LE BON USAGE DES INDICATEURS ET SOUS-ZONES NECESSAIRES SELON LE TYPE DE MENTION]" + "\n" +
		"215 ##$aImportance matérielle$cMention d'ill.$dDimensions$eMatériel d'accompagnement" + "\n" +
		"225  #$a@Titre de la Collection$xISSN$vNuméro" + "\n" +
		"300 ##$aNote générale" + "\n" +
		...
		"700 #1$aNom Auteur relatif é l'oeuvre ou é l'expression$bPrénom$4230" + "\n" +
...
{ // Ce script permet de créer une notice de périodique imprimé Ab
	...
	application.activeWindow.title.insertText(
		...
		"440 ##$t@Titre Devient" + "\n" +
		"451 ##$t@Titre Autre édition sur le méme support" + "\n" +
...
{ // Ce script permet de créer une notice de document sonore musical Ga
	...
	application.activeWindow.title.insertText(
		...
		"181 ##$P01$cprm" + "\n" + "182 ##$P01$cs" + "\n" +
		"183 ##$P01$aCode du type de support matériel" + "\n" +
		"200 1#$a@Titre$eComplément du Titre$fCompositeur principal$gAutres (compositeurs, interprétes, instrumentistes...)" + "\n" +
		"205 ##$aEdition" + "\n" +
		"214 #0$aLieu de publication$bAdresse de l'éditeur$cNom de l'éditeur$dDate de publication [CONSULTER LE GUIDE METHODOLOGIQUE POUR LE BON USAGE DES INDICATEURS ET SOUS-ZONES NECESSAIRES SELON LE TYPE DE MENTION]" + "\n" +
		"215 ##$aImportance matérielle$cMention d'ill.$dDimensions$eMatériel d'accompagnement" + "\n" +
		"225  #$a@Titre de la Collection$xISSN$vNuméro" + "\n" +
		...
		"323 ##$aNote sur les interprétes" + "\n" +
		"359 2#$vTomaison / Numérotation de l'unité physique$btable des matiéres de niveau 1$pNuméro de page ou de plage$ctable des matiéres de niveau 2" + "\n" +
		"410 ##$t@Lien au titre de la Collection$vNuméro" + "\n" +
		"606 ##$aSujet$xSubdivision de sujet$yGéographique$zChronologique$2rameau" + "\n" +
		"608 ##$aIndexation Forme ou Genre Rameau$2rameau" + "\n" +
		"700 #1$aNom Auteur relatif é l'oeuvre ou é l'expression$bPrénom$4230" + "\n" +
...
{ // Ce script permet de créer une notice de thése électronique Oa (Reproduction)
	...
	application.activeWindow.title.insertText(
		...
		"182 ##$P01$cc" + "\n" +
		"183 ##$P01$aceb" + "\n" +
		"200 1#$a@Titre$eComplément du titre$fAuteur$gsous la direction de Prénom Nom du directeur de thése" + "\n" +
		"214 #2$a $c $d" + "\n" +
		"300 ##$a(s'il y a lieu)Thése soutenue en co-tutelle" + "\n" +
		"311 ##$a(s'il y a lieu)Thése soutenue sur un ensemble de travaux" + "\n" +
		...
		"328 #0$zReproduction de$bThése d'Etat--Thése de doctorat--Thése de 3e cycle--Thése d'université--Thése de docteur-ingénieur--Thése d'exercice$cDiscipline (libellé complet)$eUniversité (voir table des libellés du Guide Méthodologique)$dAnnée de soutenance" + "\n" +
		"330 ##$aRésumé franéais$zfre" + "\n" +
		"330 ##$aRésumé anglais$zeng" + "\n" +
		"337 ##$aUn logiciel capable de lire un fichier au format (préciser le format)" + "\n" +
		"371 .#$a" + "\n" +
		"455 ##$t@Lien vers la thése originelle" + "\n" +
...
{ // Ce script permet de créer une notice de thése imprimée Aa (Document originel)corrige le 2013-09-03 MTE selon Assistance 4174
	...
	application.activeWindow.title.insertText(
		...
		"314 ##$aAutre(s) contribution(s) : Prénom Nom (Président du jury) ; Prénom Nom, Prénom Nom (Membre(s) du jury) ; Prénom Nom (Rapporteur(s))" + "\n" +
		"320 ##$aBibliographie : xxx réf." + "\n" +
		"328 #0$bThése d'Etat--Thése de doctorat--Thése de 3e cycle--Thése d'université--Thése de docteur-ingénieur--Thése d'exercice$cDiscipline (libellé complet)$eUniversité (voir table des libellés du Guide Méthodologique)$dAnnée de soutenance" + "\n" +
		"330 ##$aRésumé franéais$zfre" + "\n" +
		"330 ##$aRésumé anglais$zeng" + "\n" +
		"371 ##$a(s'il y a lieu)Thése confidentielle jusqu'au (date exacte) OU jusqu'en (année)" + "\n" +
		"541 ##$a@Titre traduit en anglais$eComplément du Titre$zeng" + "\n" +
		"600 ##$aPersonne$xSubdivision de sujet$zChronologique$2rameau" + "\n" +
		"606 ##$aAccés sujet - nom commun$2rameau" + "\n" +
		"606 ##$aACCES SUJET - NOM COMMUN$3040839486$2fmesh" + "\n" +
...
{ // Ce script permet de créer une notice de thése imprimée Aa (Edition commerciale)
	...
	application.activeWindow.title.insertText(
		...
		"215 ##$aNombre de vol. (nbr. de p. ou f.)$cMention d'ill.$dDimensions$eMatériel d'accompagnement" + "\n" +
		"320 ##$aBibliographie p.xxx-xxx. Index" + "\n" +
		"328 #0$zTexte remanié de$bThése d'Etat--Thése de doctorat--Thése de 3e cycle--Thése d'université--Thése de docteur-ingénieur--Thése d'exercice$cDiscipline (libellé complet)$eUniversité (voir table des libellés du Guide Méthodologique)$dAnnée de soutenance" + "\n" +
		"451 ##$t@Lien vers la thése originelle sur le méme support" + "\n" +
		"452 ##$t@Lien vers la thése originelle sur un support différent" + "\n" +
		"600 ##$aPersonne$xSubdivision de sujet$zChronologique$2rameau" + "\n" +
		"606 ##$aAccés sujet - nom commun$2rameau" + "\n" +
		"606 ##$aAccés sujet - nom commun$2fmesh" + "\n" +
		"610 0#$aMots clés libres$z" + "\n" +
		"700 #1$aNom Auteur$bPrénom$4070"
	);
	application.activeWindow.codedData = true;
...
{ // Ce script permet de créer une notice de document audiovisuel Ba, choisir 181
	...
	application.activeWindow.title.insertText(
		...
		"305 ##$aNote sur l'historique de l'oeuvre (ex: Enregistrement public au théétre...)" + "\n" +
		"306 ##$aNote sur l'adresse bibliographique (ex: Copyright)" + "\n" +
		"307 ##$aFormat de l'image" + "\n" +
		"312 ##$aNote sur les titres associées (ex: D'aprés le roman...)" + "\n" +
		...
		"702 #1$aNom Auteur relatif é la manifestation ou é l'item$bPrénom$4Code de Fonction" + "\n" +
		"712 02$aNom Collectivité auteur relatif é la manifestation ou é l'item$4Code de Fonction");
	application.activeWindow.codedData = true;
}
...
{ //Ce script permet de créer une notice d'article Archive
	...
	application.activeWindow.title.insertText(
		...
		"700 #1$aNom Auteur relatif é l'oeuvre ou é l'expression$bPrénom$4code de fonction" + "\n" +
		"701 #1$aCo-auteur relatif é l'oeuvre ou é l'expression$bPrénom$4code de fonction" + "\n" +
...
{ // Ce script permet de créer une proposition rameau Forme/Genre Tf3
	...
	application.activeWindow.title.insertText(
		"008 $aTf3" + "\n" +
		"280 ##$8frefre$9#y$aNouveau Point d'accés autorisé - Forme ou Genre Rameau proposé" + "\n" +
		"480 ##$8frefre$9#y$aVariante de point d'accés 1" + "\n" +
		"480 ##$8frefre$9#y$aVariante de point d'accés 2" + "\n" +
		"810 ##$aOBLIGATOIRE Référence du document pour lequel est proposé le nouveau Point d'accés Titre / Auteur, date" + "\n" +
		"810 ##$aDocument de référence permettant de justifier, définir, ... le concept proposé en Point d'accés Titre / Auteur, date"
	);
	application.activeWindow.codedData = true;
...
{ // Ce script permet de créer une notice de thése électronique Oa (version remaniée par l'auteur hors publication commerciale)
	...
	application.activeWindow.title.insertText(
		...
		"330 ##$aRésumé anglais$zeng" + "\n" +
		"337 ##$aUn logiciel capable de lire un fichier au format (préciser le format)" + "\n" +
		"45X ##$t@Lien vers la thése originelle" + "\n" +
		"541 ##$a@Titre traduit en anglais$eComplément du Titre$zeng" + "\n" +
		"600 ##$aPersonne$xSubdivision de sujet$zChronologique$2rameau" + "\n" +
		"606 ##$aAccés sujet - nom commun$2rameau" + "\n" +
		"606 ##$aAccés sujet - nom commun$3040839486$2fmesh" + "\n" +
...
{ // Ce script permet de créer une notice de mémoire nativement électronique Oa
	...
	application.activeWindow.title.insertText(
		...
		"314 ##$aAutre(s) contribution(s) : Prénom Nom (Président du jury) ; Prénom Nom, Prénom Nom (Membre(s) du jury) ; Prénom Nom (Rapporteur(s))" + "\n" +
		"320 ##$aBibliographie : xxx réf." + "\n" +
		"328 #0$bMention du travail universitaire et nature du dipléme$cDiscipline (libellé complet)$eUniversité (voir table des libellés du Guide Méthodologique)$dAnnée de soutenance" + "\n" +
		"330 ##$aRésumé franéais$zfre" + "\n" +
		"330 ##$aRésumé anglais$zeng" + "\n" +
		"337 ##$aUn logiciel capable de lire un fichier au format (préciser le format)" + "\n" +
		"541 ##$a@Titre traduit en anglais$eComplément du Titre$zeng" + "\n" +
...
{ // Ce script permet de créer une notice de mémoire électronique Oa (reproduction)
	...
	application.activeWindow.title.insertText(
		...
		"314 ##$aAutre(s) contribution(s) : Prénom Nom (Président du jury) ; Prénom Nom, Prénom Nom (Membre(s) du jury) ; Prénom Nom (Rapporteur(s))" + "\n" +
		"320 ##$aBibliographie : xxx réf." + "\n" +
		"328 #0$zReproduction de$bMention du travail universitaire et nature du dipléme$cDiscipline (libellé complet)$eUniversité (voir table des libellés du Guide Méthodologique)$dAnnée de soutenance" + "\n" +
		"330 ##$aRésumé franéais$zfre" + "\n" +
		"330 ##$aRésumé anglais$zeng" + "\n" +
		"337 ##$aUn logiciel capable de lire un fichier au format (préciser le format)" + "\n" +
...
{ // Ce script permet de créer une notice de HDR imprimé Aa
	...
	application.activeWindow.title.insertText(
		...
		"314 ##$aAutre(s) contribution(s) : Prénom Nom (Président du jury) ; Prénom Nom, Prénom Nom (Membre(s) du jury) ; Prénom Nom (Rapporteur(s))" + "\n" +
		"320 ##$aBibliographie : xxx réf." + "\n" +
		"328 #0$bHabilitation é diriger des recherches$cDiscipline (libellé complet)$eEtablissement (voir table des libellés du Guide Méthodologique)$dAnnée de soutenance" + "\n" +
		"330 ##$aRésumé franéais$zfre" + "\n" +
		"330 ##$aRésumé anglais$zeng" + "\n" +
...
{ // Ce script permet de créer une notice de HDR imprimée Oa (Reproduction)
	...
	application.activeWindow.title.insertText(
		...
		"314 ##$aAutre(s) contribution(s) : Prénom Nom (Président du jury) ; Prénom Nom, Prénom Nom (Membre(s) du jury) ; Prénom Nom (Rapporteur(s))" + "\n" +
		"320 ##$aBibliographie p.xxx-xxx. Index" + "\n" +
		"328 #0$zReproduction de$bHabilitation é diriger des recherches$cDiscipline (libellé complet)$eUniversité (voir table des libellés du Guide Méthodologique)$dAnnée de soutenance" + "\n" +
		"330 ##$aRésumé franéais$zfre" + "\n" +
		"330 ##$aRésumé anglais$zeng" + "\n" +
...
{ // Ce script permet de créer une notice de mémoire nativement électronique Oa
	...
	application.activeWindow.title.insertText(
		...
		"314 ##$aAutre(s) contribution(s) : Prénom Nom (Président du jury) ; Prénom Nom, Prénom Nom (Membre(s) du jury) ; Prénom Nom (Rapporteur(s))" + "\n" +
		"320 ##$aBibliographie : xxx réf." + "\n" +
		"328 #0$bHabilitation é diriger des recherches$cDiscipline (libellé complet)$eUniversité (voir table des libellés du Guide Méthodologique)$dAnnée de soutenance" + "\n" +
		"330 ##$aRésumé franéais$zfre" + "\n" +
		"330 ##$aRésumé anglais$zeng" + "\n" +
		"337 ##$aUn logiciel capable de lire un fichier au format (préciser le format)" + "\n" +
		"541 ##$a@Titre traduit en anglais$eComplément du Titre$zeng" + "\n" +
...
{ // Ce script permet de créer une notice de thése imprimée Aa perdue
	...
	application.activeWindow.title.insertText(
		...
		"300 ##$a(s'il y a lieu)Thése soutenue en co-tutelle" + "\n" +
		"303 ##$aNotice élaborée é partir de la reproduction de la thése : la version de soutenance n'existe plus. OU Notice élaborée é partir de [mention des sources] : la version de soutenance n'existe plus.OU Notice élaborée é partir de [mention de sources]. Le document qui a justifié l'obtention du dipléme n'existe plus." + "\n" +
		"311 ##$a(s'il y a lieu)Thése soutenue sur un ensemble de travaux" + "\n" +
		"314 ##$aEcole(s) doctorale(s) : Nom de l'école doctorale" + "\n" +
		"314 ##$aPartenaire(s) de recherche : Nom du Laboratoire (Laboratoire) ; Nom de l'équipe de recherche (Equipe de recherche) ; Nom de l'entreprise (Entreprise) ; Nom de la fondation (Fondation) ; Nom d'un autre partenaire (Expliciter le type de partenaire)" + "\n" +
		"314 ##$aAutre(s) contribution(s) : Prénom Nom (Président du jury) ; Prénom Nom, Prénom Nom (Membre(s) du jury) ; Prénom Nom (Rapporteur(s))" + "\n" +
		...
		"330 ##$aRésumé franéais$zfre" + "\n" +
		"330 ##$aRésumé anglais$zeng" + "\n" +
		"371 ##$a(s'il y a lieu)Thése confidentielle jusqu'en (année)" + "\n" +
		"541 ##$a@Titre traduit en anglais$eComplément du Titre$zeng" + "\n" +
		"600 ##$aPersonne$xSubdivision de sujet$zChronologique$2rameau" + "\n" +
		"606 ##$aAccés sujet - nom commun$2rameau" + "\n" +
		"606 ##$aAccés sujet - nom commun$3040839486$2fmesh" + "\n" +
		...
		"711 02$a@Laboratoire$4981" + "\n" +
		"711 02$a@Autre type de partenaire (composante, institut, établissement d'inscription, etc)$4985" + "\n" +
		"E316 ##$aExemplaire manquant (constaté en AAAA) OU Exemplaire manquant/incommunicable depuis AAAA, pour cause de ... OU Exemplaire manquant/incommunicable depuis AAAA, pour cause de .... Il n'existe pas de copie conforme (au contenu strictement identique) de la version de soutenance$ulien vers la notice Sudoc de la reproduction conforme OU de la version non corrigée / remaniée / commercialisée de la thése$2Consulter la reproduction OU Consulter la version non corrigée de la thése OU Consulter la version commercialisée de la thése OU Consulter une version de la thése remaniée par l'auteur"
	);
	application.activeWindow.codedData = true;
}
...
{ // Ce script permet de créer une notice de thése imprimée Aa non déposée
	...
	application.activeWindow.title.insertText(
		...
		"215 ##$a(si informations disponibles)x vol. (xxx p.)$dDimensions" + "\n" +
		"300 ##$a(s'il y a lieu)Thése soutenue en co-tutelle" + "\n" +
		"303 ##$aNotice élaborée é partir de la version non corrigée de la thése : la version de soutenance n'a pas été déposée OU Description établie é partir de (préciser les documents administratifs utilisés pour établir la description)" + "\n" +
		"305 ##$a(s'il y a lieu) En l'absence de dépét de la version définitive de la thése, la version non corrigée fait office de dépét légal. OU (si thése non déposée) La version de soutenance n'existe pas. Le docteur n'a déposé aucun exemplaire de sa thése pour archivage et communication." + "\n" +
		"311 ##$a(s'il y a lieu)Thése soutenue sur un ensemble de travaux" + "\n" +
		"314 ##$aEcole(s) doctorale(s) : Nom de l'école doctorale" + "\n" +
		"314 ##$aPartenaire(s) de recherche : Nom du Laboratoire (Laboratoire) ; Nom de l'équipe de recherche (Equipe de recherche) ; Nom de l'entreprise (Entreprise) ; Nom de la fondation (Fondation) ; Nom d'un autre partenaire (Expliciter le type de partenaire)" + "\n" +
		"314 ##$aAutre(s) contribution(s) : Prénom Nom (Président du jury) ; Prénom Nom, Prénom Nom (Membre(s) du jury) ; Prénom Nom (Rapporteur(s))" + "\n" +
		...
		"711 02$a@Laboratoire$4981" + "\n" +
		"711 02$a@Autre type de partenaire (composante, institut, établissement d'inscription, etc)$4985" + "\n" +
		"E316 ##$aExemplaire manquant : le docteur n'a jamais déposé la version corrigée, validée, de sa thése. OU Exemplaire manquant : le docteur n'a jamais déposé la version validée en soutenance de sa thése. OU Exemplaire manquant : le docteur n'a jamais déposé sa thése.$u(s'il y a lieu)lien vers la notice Sudoc de la version non corrigée--commercialisée--remaniée par l'auteur$2Consulter la version non corrigée de la thése OU Consulter la version remaniée et commercialisée de la thése OU Consulter la version de la thése remaniée par l'auteur."
	);
	application.activeWindow.codedData = true;
}
