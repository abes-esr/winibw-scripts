...
if (affichage == "K") ppnv = "";
if (ppnv != "") NbRes = 1;
if (NbRes>1) result = prompts.prompt(null, "Correspondants", "Attention vous �tes sur un lot de notices, voulez vous le parcourir enti�rement ? (oui/non)", input,"", check);
if (input.value == "non") {
    if (ppnv == "") NbRes = 0;
...
for (courrant=deb; courrant<=NbRes; courrant++) {
		...
		chaine = "Liste des correspondants catalogage des " + cpt + " biblioth�que(s) localis\u00e9e(s) sous la notice ppn " + PPN +",\r\n";
		fichierSortie.write(chaine, chaine.length);
		var mois = 0;
		mois = jour.getMonth();
		mois = mois + 1;
		...
		fichierSortie.write(chaine, chaine.length);
		chaine = "\r\n***** Attention *********, il faut copier les adresses bloc par bloc dans la messagerie (un bloc=1000 caract�res maximum), le copier-coller de l'ensemble pose probl�me avec certains logiciels de messagerie ********\r\n\r\n\r\n\r\n";
		fichierSortie.write(chaine, chaine.length);
		chaine = "";
		reste = 0;
		for (i=0; i<cpt; i++) {
