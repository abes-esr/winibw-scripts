//2014-10-09 mte : verifie pour 181/182
function AFF_S()
{
...
function AFF_OJ()
{
// affiche les exemplaires classes par date (tout le reseau)
	application.activeWindow.command("aff oj", false);
}
function AFF_OB()
{
	// affiche les exemplaires classes par n°RCR (tout le reseau)
	application.activeWindow.command("aff ob", false);
}
function AFF_KZone()
{
	// affiche les zones demandees de la ou les notices en format Unimarc
	application.activeWindow.commandLine = "aff K:";
}
function AFF_I()
{
	// affiche les notices sous la forme normalisee ISBD
...
function AFF_U()
{
	// affiche les notices sous la forme libellee
