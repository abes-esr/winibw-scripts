...
function CreexeNumero() {
	...
	if (application.activeWindow.getVariable("scr") == "8A") {
		...
		if (gPicaUtility.isAuthority(matCode)) {
			application.messageBox("WinIBW 3", "C'est une notice autorité, pas d'exemplaire!", "message-icon");
			return;
		}
		// Verify that it does not have b-status:
		if (matCode.charAt(2) == 'B') {
			application.messageBox("WinIBW 3", "C'est une notice en statut 'B', impossible de créer un exemplaire!", "message-icon");
			return;
		}
